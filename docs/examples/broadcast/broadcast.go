...
func main() {
	...
	_, err = management.DeclareExchange(context.TODO(), &rmq.FanOutExchangeSpecification{
	...
	for i := 0; i < 5; i++ {
		// create temp queues
		q, err := management.DeclareQueue(context.TODO(), &rmq.AutoGeneratedQueueSpecification{
			IsAutoDelete: true,
			IsExclusive:  true,
		})
